...
async function buildPdf(filename, analysis) {
  ...
  const text = (function analysisToText(analysis) {
    if (!analysis || typeof analysis !== "object") return "No analysis provided.";
    const parts = [];
    if (analysis.executiveSummary) { parts.push("Executive Summary:", analysis.executiveSummary, ""); }
    if (Array.isArray(analysis.keyMetrics) && analysis.keyMetrics.length) {
...
export default async function handler(req, res) {
  try {
    ...
    const { pdfDataUrl, language = "eng", filename = "Uploaded_PDF_Report" } = body || {};
    if (!pdfDataUrl || !pdfDataUrl.startsWith("data:application/pdf;base64,")) {
      return res.status(400).json({ ok:false, error:"Provide 'pdfDataUrl' (data:application/pdf;base64,...) in body" });
    }

    // 1) OCR (base64 route)
    const text = await runOCRBase64(pdfDataUrl, language);

    // 2) Analyze
    const analysis = await analyzeText(text, { source: "upload" });
